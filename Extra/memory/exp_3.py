...
for i in range(5):
    mpI = nest.Create('music_event_in_proxy', {'port_name': 'in'})
    nest.SetStatus(mpI, {'music_channel': i})
    mpIn.append(mpI)

for i in range(5):
    mpOA = nest.Create('music_event_out_proxy', {'port_name': 'out'})
    mpOutAct.append(mpOA)

for i in range(3):
    mpOO = nest.Create('music_event_out_proxy', {'port_name': 'out'})
    mpOutObj.append(mpOO)
...
for i in range(5): 
    ...
    nest.Connect(neuOutAct[i], mpOutAct[i], "one_to_one",{'music_channel': i})
...
for i in range(3):
    nest.Connect(neuOutObj[i], mpOutObj[i], "one_to_one",{'music_channel': (4+i)})
